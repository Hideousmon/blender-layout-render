...
class MultiLinesFlow:
    def __init__(self, point3d_list, lines_number = 26, material1=None, material2=None, rename=None, rotation_degree = 90,
                 emission_strength = 10, resolution_u = 24):
    ...
    def draw(self):
        for j in range(self.lines_number):
            curve = bpy.data.curves.new(f"Lines_Inner_{j}", type='CURVE')
            curve.dimensions = '3D'
            curve.resolution_u = self.resolution_u
            curve.bevel_depth = random.uniform(0.008, 0.05)
            curve.bevel_resolution = 8
            curve.fill_mode = 'FULL'

            sp = curve.splines.new('BEZIER')
            sp.bezier_points.add(len(self.point_list) - 1)
            ...
            for i, p in enumerate(self.point_list):
                ...
                bp.co = (x + dx, y + dy, z + dz)
                bp.handle_left_type = 'AUTO'
                bp.handle_right_type = 'AUTO'

            obj = bpy.data.objects.new(f"Lines_Inner_Obj_{j}", curve)
            ...
            if not self.material1 is None:
                ...
                color_ramp_node.color_ramp.elements[0].position = 0.5
